...
async function addJob() {
  const urlToText = document.querySelector('#url-to-text').value;
...
async function updateJobs() {
  for (let id of Object.keys(jobs)) {
    ...
    let asTextRes = await fetch(`/job/${id}`);
    let asText = await asTextRes.text();
    if (!!jobs[id]) {
      jobs[id] = {
        ...result,
        asText
